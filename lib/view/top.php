...
<?php require DIR_VIEW_ELEMENT . 'output_navber.php'; ?>
	<div class="container-fluid px-md-5">
		...
		<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="get">
    		<select name="action">
    			<option value="new_item"　<?php if($get_action === "new_item"){ echo 'selected';} ?>>最新商品</option>
    			<option value="cheap_item" <?php if($get_action === "cheap_item"){ echo 'selected';} ?>>価格の安い順</option>
    			<option value="expensive_item" <?php if($get_action === "expensive_item"){ echo 'selected';} ?>>価格の高い順</option>
    		</select>
...
<?php } ?>
	...
	<?php if($page > 1){ ?>
	<a class="btn btn-primary cart-btn" href="./top.php?page=<?php echo h($page - 1); ?>&action=<?php echo $get_action; ?>">前へ</a>
	<?php  } ?>
	<?php for($p=1;$p<=$max_page;$p++){ ?>
		<?php if($p === (int)$page){ ?>
			<div class="btn btn-info cart-btn"><?php echo h($p); ?></div>
		<?php }else{  ?>
			<a class="btn btn-primary cart-btn" href="./top.php?page=<?php echo h($p); ?>&action=<?php echo $get_action; ?>"><?php echo h($p); ?></a>
		<?php  } ?>
	<?php  } ?>
	<?php if($page < $max_page){ ?>
	<a class="btn btn-primary cart-btn" href="./top.php?page=<?php echo h($page + 1); ?>&action=<?php echo $get_action; ?>">次へ</a>
	<?php  } ?>
	</div>
	<!-- /.container -->
